...
def parse_git_log(file_path, exclude_patterns=[]):
        ...
        for line in file:
            ...
            if line.startswith("--"):
                ...
                if match and current_commit:
                    ...
                    # # Add file change details to the 'details' field
                    # if change_type == 'R':
                    #     # Add the rename details (old_path, new_path)
    ...
    # Convert file_exts set to list for each commit
    for commit in commits:
        commit['file_exts'] = list(commit['file_exts'])
...
exclude_patterns = ['locale/*/*/*.po', "*/migrations/*"]
parsed_commits = parse_git_log(log_file, exclude_patterns)

# Pretty print results
for commit in parsed_commits[:10]:
    print(commit)

print(f'done, {time.time() - c1}')
